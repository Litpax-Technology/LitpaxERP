...
function ensureCustEditModal() {
  if (document.getElementById('custEditModal')) return;
  document.body.insertAdjacentHTML('beforeend', `
  <div class="modal-overlay" id="custEditModal">
    <div style="background:#fff;border-radius:14px;width:520px;max-width:94vw;max-height:90vh;overflow:auto;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #e5e7eb;">
        <div><div style="font-size:15px;font-weight:700;">📝 Edit Customer</div>
        <div id="ce-id-display" style="font-size:12px;color:#888;font-family:monospace;"></div></div>
        <button onclick="closeModal('custEditModal')" style="background:none;border:none;font-size:18px;cursor:pointer;color:#888;">✕</button>
      </div>
      <div style="padding:18px 20px;">
        <input type="hidden" id="ce-custid">
        <div class="form-group"><label class="form-label">Company Name</label><input class="form-control" id="ce-name"></div>
        <div class="form-group" style="margin-top:12px;"><label class="form-label">Contact Person</label><input class="form-control" id="ce-contact"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
          <div class="form-group"><label class="form-label">Phone</label><input class="form-control" id="ce-phone"></div>
          <div class="form-group"><label class="form-label">City</label><input class="form-control" id="ce-city"></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
          <div class="form-group"><label class="form-label">GSTIN</label><input class="form-control" id="ce-gst"></div>
          <div class="form-group"><label class="form-label">Credit Days</label><input class="form-control" id="ce-credit"></div>
        </div>
      </div>
      <div style="display:flex;justify-content:flex-end;gap:10px;padding:14px 20px;border-top:1px solid #e5e7eb;">
        <button class="btn" onclick="closeModal('custEditModal')">Cancel</button>
        <button class="btn btn-primary" id="ce-save-btn" onclick="submitCustEdit()">💾 Save</button>
      </div>
    </div>
  </div>`);
}
function openCustEdit(c) {
  ensureCustEditModal();
  document.getElementById('ce-custid').value = c.CustomerID || '';
  document.getElementById('ce-id-display').textContent = c.CustomerID || '';
  document.getElementById('ce-name').value = c.CompanyName || '';
  document.getElementById('ce-contact').value = c.ContactPerson || '';
  document.getElementById('ce-phone').value = c.Phone || '';
  document.getElementById('ce-city').value = c.City || '';
  document.getElementById('ce-gst').value = c.GSTIN || '';
  document.getElementById('ce-credit').value = (c.CreditDays === 0 || c.CreditDays) ? c.CreditDays : '';
  openModal('custEditModal');
}
function submitCustEdit() {
  const id = document.getElementById('ce-custid').value;
  if (!id) { toast('CustomerID nahi mila', 'e'); return; }
  const name = document.getElementById('ce-name').value.trim();
  if (!name) { toast('Company Name zaroori hai', 'e'); return; }
  ...
  api({
    ...
    CustomerID: id,
    CompanyName: name,
    ContactPerson: document.getElementById('ce-contact').value.trim(),
    Phone: document.getElementById('ce-phone').value.trim(),
    GSTIN: document.getElementById('ce-gst').value.trim().toUpperCase(),
    City: document.getElementById('ce-city').value.trim(),
    CreditDays: document.getElementById('ce-credit').value.trim()
  }, r => {
    if (btn) { btn.disabled = false; btn.textContent = '💾 Save'; }
    if (r.success) { toast('Customer updated!'); closeModal('custEditModal'); loadCustomers(); }
    else toast(r.message || 'Update failed', 'e');
  });
}
